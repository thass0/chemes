...
pub fn eval(env: &mut Env, value: Value) -> EvalResult<Value> {
    match value {
        ...
        Value::Symbol(s) => env.lookup(&s).ok_or_else(|| EvalError::Undefined(s)),
        Value::Pair { car, cdr } => match *car {
            Value::Symbol(ref s) => match s.as_str() {
                ...
                "cond" => todo!(),
                _ => apply(eval(env, *car)?, eval_list(env, *cdr)?),
            },
            _ => apply(eval(env, *car)?, eval_list(env, *cdr)?),
        },
    ...
    match target {
        Value::Pair { car, cdr } => {
            if let Value::Symbol(sym) = *car {
                ...
                let lambda = Value::list(&[Value::symbol("lambda"), *cdr, body]);
...
fn eval_list(env: &mut Env, value: Value) -> EvalResult<Value> {
    match value {
        Value::Pair { car, cdr } => Ok(eval(env, *car)?.cons(&eval_list(env, *cdr)?)),
        Value::Empty => Ok(Value::Empty),
...
mod tests {
    ...
    fn self_evaluating_values_self_evaluate() {
        ...
        assert_eq!(eval(&mut e, Value::True).unwrap(), Value::True,);
        assert_eq!(eval(&mut e, Value::False).unwrap(), Value::False,);
    }
    ...
    fn evaluating_set_works() {
        ...
        assert_eq!(
            eval(&mut e, Value::list(&[Value::symbol("set!"),]),).unwrap_err(),
    ...
    fn evaluating_define_works() {
        ...
        assert_eq!(
            ...
            Value::list(&[
                Value::symbol("lambda"),
                Value::list(&[Value::symbol("x"), Value::symbol("y"),]),
                Value::symbol("y"),
            ]),
        );
